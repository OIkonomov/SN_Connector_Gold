...
WHERE 
    CLIENT_TIME >= '2020-08-14'
    AND CLIENT_TIME <= '2020-09-15'
    AND DATA_CENTER_ID LIKE 'eur%'
    AND REALM_ID = 288
    AND (LOST NOT LIKE '%"3073%' OR WOUNDED NOT LIKE '%"3073%')
    AND COMBAT_INT NOT LIKE 'Defend%'
;
'''        
# print(SQL_REQ)
...
if len(q_result) != 0:
    ...
    lost_df = lost_df[~lost_df.UNITS.isin(mercs_list)]
    ...
    w_df = w_df[~w_df.UNITS.isin(mercs_list)]
    ...
    units_df.to_csv("not_mercs_attack.csv")
